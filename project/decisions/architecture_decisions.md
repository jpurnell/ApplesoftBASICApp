# Architecture Decisions — ApplesoftBASICApp
...
decision: |
  ...
  DOCUMENT → VERIFY. Design proposals live in project/plans/proposals/.
rationale: |
...
affected_files:
  - development-guidelines/rules/design_proposal.md
  - development-guidelines/rules/session_workflow.md
  - development-guidelines/templates/checklist.md
supersedes: null
amends: null
superseded_by: null
```
...
affected_files:
  - development-guidelines/rules/session_workflow.md
  - project/summaries/development-guidelines/templates/session_summary.md
supersedes: null
amends: null
superseded_by: null
```
...
affected_files:
  - development-guidelines/rules/docc_guidelines.md
  - development-guidelines/rules/design_proposal.md
supersedes: null
amends: null
superseded_by: null
...
affected_files:
  - development-guidelines/rules/session_workflow.md
  - development-guidelines/templates/checklist.md
  - development-guidelines/rules/RELEASE_CHECKLIST.md
...
decision: |
  Use a single structured YAML-block log (architecture_decisions.md)
...
decision: |
  ...
  project/checklists/. Completed features move to 04_99_COMPLETED/,
...
affected_files:
  - development-guidelines/rules/no_hardcoded_constants.md
supersedes: null
amends: null
superseded_by: null
```
...
context: |
  The original setup.swift created project/plans/,
  project/checklists/, and project/summaries/ at the consuming
...
consequences: |
  ...
  - Paths in skills/CLAUDE.md are slightly longer (development-guidelines/project/summaries/)
alternatives_rejected:
...
status: accepted
category: tooling
title: ContextAuditor as advisory ethical context checker in quality-gate
context: |
  Quality gates catch code correctness issues but not ethical context issues —
  accessing sensitive APIs without consent guards, tracking users without opt-out
  checks, making automated deny/block/suspend decisions without human review,
  or enabling background location tracking without disclosure. These patterns
  are legal and compliance risks that pass all existing checkers.
decision: |
  Add a ContextAuditor to quality-gate-swift that uses SwiftSyntax to detect
  four categories of ethical risk: missing consent guards, unguarded analytics,
  automated decisions without review, and surveillance patterns. Each rule can
  be suppressed with a justification annotation (// CONSENT:, // ANALYTICS:,
  // REVIEWED:, // DISCLOSURE:). Silent suppression is not allowed. The checker
  returns .warning, never .failed — it is advisory-only and designed to be
  easily disableable if not delivering value.
rationale: |
  - Ethical context issues are invisible to standard linters and type checkers
  - Advisory-only status prevents warning fatigue and gate avoidance
  - Justification annotations create auditable records of deliberate choices
  - Consent guard detection scans only guard/if lines to avoid false positives
    from API method names like requestAuthorization
  - Warning fatigue philosophy: ignored warnings erode gate trust, so advisory
    checkers should earn their keep or be removed
consequences: |
  + Ethical risks surface during development, not after deployment
  + Justification annotations create an audit trail
  + Advisory status means teams opt into enforcement, not forced
  - False positives possible for unconventional code patterns
  - SwiftSyntax dependency adds to quality-gate build time
alternatives_rejected:
  - "Hard failure on ethical violations: Creates adversarial relationship with the gate"
  - "Comment-based suppression without justification text: No audit trail"
  - "Runtime detection only: Too late — the code is already shipped"
affected_files:
  - quality-gate-swift/Sources/ContextAuditor/ContextAuditor.swift
  - quality-gate-swift/Sources/ContextAuditor/ContextVisitor.swift
  - quality-gate-swift/Sources/QualityGateCLI/QualityGateCLI.swift
supersedes: null
amends: null
superseded_by: null
